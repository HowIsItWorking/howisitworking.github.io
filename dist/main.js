...
const deactivateCheckBox = document.getElementById("deactivate-background");
...
function getHSLAString(colorObj, alpha) {
    ...
    for (let i = 0; i < dots.length; i++) {
        ...
        for (let j = i + 1; j < dots.length; j++) {
            ...
            for (let k = j + 1; k < dots.length; k++) {
                ...
                if (triangles.length >= MAX_TRIANGLES)
                    return;
                if (distance(a, c) >= MAX_DISTANCE || distance(b, c) >= MAX_DISTANCE)
                    continue;
...
function applyPhysics() {
    ...
    for (let i = 0; i < dots.length; i++) {
        ...
        a.vx *= FRICTION;
        a.vy *= FRICTION;
        if (a.x <= 0 || a.x >= rect.width)
            a.vx *= -1;
        if (a.y <= 0 || a.y >= rect.height)
            a.vy *= -1;
    }
}
// ===================== RENDER =====================
