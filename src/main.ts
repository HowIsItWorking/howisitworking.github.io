...
interface Triangle { a: number; b: number; c: number; color: string; }
...
Array.from(anchors).forEach(anchor => {
  anchor.addEventListener("mouseenter", () => {
    ...
    if (anchor.classList.contains("linkedin")) triangleColor = linkedinColor;
    else if (anchor.classList.contains("github")) triangleColor = githubColor;
    else if (anchor.classList.contains("instagram")) triangleColor = instagramColor;
  });
...
function isTriangleValid(t?: Triangle): boolean {
  if (!t) return false;
...
function trianglesOverlap(t1: Triangle, a1?: Dot, b1?: Dot, c1?: Dot): boolean {
  if (!a1 || !b1 || !c1) return false;
  const t2a = dots[t1.a], t2b = dots[t1.b], t2c = dots[t1.c];
  if (!t2a || !t2b || !t2c) return false;
  ...
  for (const axis of axes) {
    if (!axis || axis.length < 2) continue; // evita undefined
    if (!axis || axis.length < 2 || axis[0] == null || axis[1] == null) continue;
    const ax = -axis[1];
    const ay = axis[0];
    if (ax == null || ay == null) continue; // checagem extra
    ...
    if (max1 < min2 || max2 < min1) return false; // separating axis encontrado
  }
  return true;
}
...
function buildTriangles(): void {
  for (let i = 0; i < dots.length; i++) {
    ...
    for (let j = i + 1; j < dots.length; j++) {
      const b = dots[j];
      if (!b || distance(a, b) >= MAX_DISTANCE) continue;

      for (let k = j + 1; k < dots.length; k++) {
        const c = dots[k];
        if (!c) continue;
        if (distance(a, c) >= MAX_DISTANCE) continue;
        if (distance(b, c) >= MAX_DISTANCE) continue;
...
function drawAll(): void {
  ...
  applyDotRepulsion(); applyMouseRepulsion(); updateDots();

  for (let i = triangles.length - 1; i >= 0; i--) {
    if (!isTriangleValid(triangles[i])) {
      const t = triangles[i];
      if (t) triangleKeys.delete(triangleKey(t.a, t.b, t.c));
...
function loop(): void {
  if (active) { canvas.style.visibility = "visible"; drawAll(); }
...
// ===================== INIT =====================
if (!initialized) {
  for (let i = 0; i < MAX_DOTS_FINAL; i++) dots.push(createDot());
  initialized = true;
  loop();
}
